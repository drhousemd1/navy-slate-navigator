...
const Rules: React.FC = () => {
    ...
    const { isLoading, error, data: rules } = useQuery({
        queryKey: ['rules'],
        queryFn: fetchRules,
    ...
    const handleRuleBroken = async (rule: Rule) => {
        try {
            ...
            queryClient.setQueryData<Rule[]>(['rules'], (old) =>
                old?.map((r) => (r.id === rule.id ? { ...r, usage_data: newUsageData } : r))
            );
    ...
    const handleSaveRule = async (ruleData: Partial<Rule>) => {
        try {
            ...
            if (ruleData.id) {
                ...
                // Optimistically update the cache
                queryClient.setQueryData<Rule[]>(['rules'], (old) =>
                    old?.map((rule) => (rule.id === ruleData.id ? { ...rule, ...result } : rule))
                );
                ...
                <div className="container mx-auto px-4 py-6">
                    ...
                    {isLoading ? (
                        ...
                        <div className="text-center py-10">
                            <p className="text-white mb-4">Error fetching rules.</p>
                        </div>
                    ) : rules ? (
