...
interface TaskEditorFormProps {
  ...
  onCancel: () => void;
  initialCarouselTimer?: number;  // For global timer value
  updateCarouselTimer?: (timer: number) => void; // To update global timer
}
...
const TaskEditorForm: React.FC<TaskEditorFormProps> = ({ 
  ...
  const form = useForm<TaskFormValues>({
    defaultValues: {
      ...
      carousel_timer: taskData?.carousel_timer || initialCarouselTimer || 5,
    },
  });

  // Sync with global timer if task doesn't have its own timer
  useEffect(() => {
    if (initialCarouselTimer && !taskData?.carousel_timer) {
      form.setValue('carousel_timer', initialCarouselTimer);
    }
  }, [initialCarouselTimer, form, taskData]);
  ...
  const handleSubmit = async (values: TaskFormValues) => {
    ...
    try {
      ...
      // Update global carousel timer if needed
      if (values.carousel_timer && updateCarouselTimer) {
        updateCarouselTimer(values.carousel_timer);
      }
  ...
  const handleCarouselTimerChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = parseInt(e.target.value);
    if (!isNaN(value) && value > 0) {
      form.setValue('carousel_timer', value);
      
      // Update global timer in real-time if this is requested
      if (updateCarouselTimer) {
        updateCarouselTimer(value);
      }
    }
  };
            ...
            <FormItem>
              ...
              <FormControl>
                <Input 
                  ...
                  className="bg-dark-navy border-light-navy text-white" 
                  {...field}
                  onChange={(e) => handleCarouselTimerChange(e)}
                  value={field.value || 5}
                />
              </FormControl>
            </FormItem>
