...
const RuleEditModal: React.FC<RuleEditModalProps> = ({
  ...
  const form = useForm<RuleFormValues>({
    defaultValues: {
      ...
      frequency: cardData?.frequency || 'daily',
      frequency_count: cardData?.frequency_count || 1
    }
  });
  
  useEffect(() => {
    if (isOpen && cardData) {
      console.log("Rule Modal opened with card data:", cardData);
      form.reset({
        id: cardData.id,
        title: cardData.title,
        description: cardData.description,
  ...
  const handleDeleteRule = () => {
    try {
      console.log("Deleting rule", cardData.id);
