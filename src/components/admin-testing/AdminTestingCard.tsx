...
const AdminTestingCard: React.FC<AdminTestingCardProps> = ({
  ...
  return (
    <>
      <Card className={`relative overflow-hidden border-2 ${isReorderMode ? 'border-amber-500 draggable-card' : 'border-[#00f0ff]'} bg-navy`}>
        {isReorderMode && (
          <div className="absolute top-2 right-2 z-30 bg-amber-500 text-white p-1 rounded-md flex items-center drag-handle">
        ...
        />
        <div className={`relative z-20 flex flex-col p-4 md:p-6 h-full ${isReorderMode ? 'pointer-events-none' : 'pointer-events-auto'}`}>
