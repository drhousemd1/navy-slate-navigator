...
const TaskEditorForm: React.FC<TaskEditorFormProps> = ({ 
  ...
  const form = useForm<TaskFormValues>({
    defaultValues: {
      ...
      focal_point_y: 50,
      priority: 'medium',
      image_meta: null, // Add image_meta default
    },
  });
  ...
  const { clearPersistedState } = useFormStatePersister(persisterFormId, form, {
    exclude: ['background_image_url', 'icon_url', 'image_meta'] // Exclude image_meta from persistence
  });
  ...
  useEffect(() => {
    if (taskData) {
      reset({
        ...
        image_meta: (taskData as any).image_meta || null, // Include existing image_meta
      });
      ...
      reset({ 
        ...
        highlight_effect: false, focal_point_x: 50, focal_point_y: 50, priority: 'medium', image_meta: null
      });
