...
const AppLayout: React.FC<AppLayoutProps> = ({ children, onAddNewItem }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-dark-navy prevent-overscroll">
      ...
      {/* Main content with simple fixed padding */}
      <main className={`flex-1 pt-[calc(4rem+env(safe-area-inset-top))] ${isMessagesPage ? 'pb-0' : 'pb-16'} overflow-y-auto animate-fade-in allow-scroll-y`}>
        {children}
      </main>
      
      {shouldShowAddButton && !isMessagesPage && (
        <div className="fixed left-0 right-0 flex justify-center py-2 z-40 bottom-20">
          <Button 
