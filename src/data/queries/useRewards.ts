...
async function fetchRewardsWithCache(): Promise<Reward[]> {
  ...
  if (!shouldFetch && localData) {
    console.log('[useRewards] Returning rewards from IndexedDB, found', localData.length, 'rewards');
    return localData;
  }
  ...
  try {
    ...
    if (data) {
      ...
      console.log('[useRewards] Successfully fetched', rewardsData.length, 'rewards from server');
      
      try {
        await saveRewardsToDB(rewardsData);
        await setLastSyncTimeForRewards(new Date().toISOString());
      } catch (err) {
        console.error('[useRewards] Error saving to IndexedDB:', err);
      }
    ...
    console.error('[useRewards] Exception in fetchRewards:', fetchError);
    if (localData) {
      return localData;
    }
    throw fetchError;
