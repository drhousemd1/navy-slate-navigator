...
const fetchMonthlyData = async (): Promise<MonthlyMetricsData> => {
  logger.log('[fetchMonthlyData] Fetching monthly chart data via useMonthlyMetrics hook at', new Date().toISOString());
  ...
  try {
    ...
    if (taskError) logger.error('[fetchMonthlyData] Error loading task_completion_history for monthly hook', { message: taskError.message });
    else if (taskEntries) {
    ...
    if (ruleError) logger.error('[fetchMonthlyData] Error loading rule_violations for monthly hook', { message: ruleError.message });
    else if (ruleEntries) {
    ...
    const { data: rewardEntries, error: rewardError } = await supabase
      ...
      .lte('created_at', end);
    if (rewardError) logger.error('[fetchMonthlyData] Error loading reward_usage for monthly hook', { message: rewardError.message });
    ...
    if (punishmentError) logger.error('[fetchMonthlyData] Error loading punishment_history for monthly hook', { message: punishmentError.message });
    ...
    logger.log('[fetchMonthlyData] Monthly chart data prepared via hook. Monthly totals:', { monthlyTotals });
    return { dataArray, monthlyTotals };
  } catch (err: any) {
    logger.error('[fetchMonthlyData] Error in fetchMonthlyData in hook:', { message: err?.message });
    toast({
