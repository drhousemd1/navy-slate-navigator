...
const fetchUserDomPoints = async (userId: string | null): Promise<number> => {
  if (!userId) {
    logger.log('fetchUserDomPoints: No userId provided, returning 0.');
    return 0;
  }

  try {
    // Attempt to fetch fresh data from Supabase
    logger.log(`fetchUserDomPoints: Fetching DOM points from Supabase for user ${userId}.`);
    ...
    if (supabaseError) {
      ...
      logger.log(`fetchUserDomPoints: Loaded ${cachedDomPoints ?? 0} DOM points from IndexedDB for fallback.`);
      return cachedDomPoints ?? 0;
    }

    if (supabaseData) {
      const domPointsToSave = supabaseData.dom_points ?? 0;
      logger.log(`fetchUserDomPoints: Successfully fetched ${domPointsToSave} DOM points from Supabase for user ${userId}. Saving to IndexedDB.`);
    ...
    const cachedDomPoints = await loadDomPointsFromDB();
    logger.log(`fetchUserDomPoints: Loaded ${cachedDomPoints ?? 0} DOM points from IndexedDB after exception.`);
    return cachedDomPoints ?? 0;
  }
};
