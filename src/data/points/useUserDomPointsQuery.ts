...
const fetchUserDomPoints = async (userId: string | null): Promise<number> => {
  if (!userId) {
    console.log('fetchUserDomPoints: No userId provided, returning 0.');
    return 0;
  }

  try {
    // Attempt to fetch fresh data from Supabase
    console.log(`fetchUserDomPoints: Fetching DOM points from Supabase for user ${userId}.`);
    ...
    if (supabaseError) {
      console.error(`fetchUserDomPoints: Supabase error for user ${userId}: ${supabaseError.message}. Falling back to IndexedDB.`);
      const cachedDomPoints = await loadDomPointsFromDB(); // Assumes this loads points for the 'dom' user context
      console.log(`fetchUserDomPoints: Loaded ${cachedDomPoints ?? 0} DOM points from IndexedDB for fallback.`);
      return cachedDomPoints ?? 0;
    }

    if (supabaseData) {
      const domPointsToSave = supabaseData.dom_points ?? 0;
      console.log(`fetchUserDomPoints: Successfully fetched ${domPointsToSave} DOM points from Supabase for user ${userId}. Saving to IndexedDB.`);
      await saveDomPointsToDB(domPointsToSave); // Assumes this saves points for the 'dom' user context
      return domPointsToSave;
    }

    console.warn(`fetchUserDomPoints: No data from Supabase for user ${userId} but no error. Attempting IndexedDB fallback.`);
    const cachedDomPoints = await loadDomPointsFromDB();
    return cachedDomPoints ?? 0;

  } catch (e: any) {
    console.error(`fetchUserDomPoints: Exception for user ${userId}: ${e.message}. Falling back to IndexedDB.`);
    const cachedDomPoints = await loadDomPointsFromDB();
    console.log(`fetchUserDomPoints: Loaded ${cachedDomPoints ?? 0} DOM points from IndexedDB after exception.`);
    return cachedDomPoints ?? 0;
  }
};
