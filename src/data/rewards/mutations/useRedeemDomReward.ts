...
// Kept in this file for now to minimize change scope, consider moving to a util if used elsewhere.
...
export const useRedeemDomReward = () => {
  ...
  return useMutation<
    { success: boolean; rewardId: string }, // Type of data returned by mutationFn
    Error, // Type of error
    RedeemDomRewardVariables, // Type of variables passed to mutationFn
    RedeemRewardOptimisticContext // Type of context passed from onMutate to onError/onSettled
  >({
    mutationFn: async ({ rewardId, currentSupply, profileId }) => {
      ...
      const { error: usageError } = await supabase
        ...
        .insert([{
          ...
          used: false, // Assuming 'used: false' means it's redeemed but not yet 'consumed' if there's such a distinction.
    ...
    onSettled: (data, error, variables) => {
      ...
      // Invalidate the specific reward query as well
