...
serve(async (req: Request) => {
  try {
    ...
    if (body.mode === 'ping') {
      ...
      return new Response(JSON.stringify({ ok:true, mode:'ping' }), {status:200});
    }

    const endpoint = body.endpoint;
    if (!endpoint) {
      console.log('No endpoint provided in test body. Returning 400.');
      return new Response(JSON.stringify({ error:'endpoint required for direct test'}), {status:400});
    }
